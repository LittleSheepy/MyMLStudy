...
MySELUPlugin::MySELUPlugin(const std::string name, const std::string attr1, float attr3)
    ...
    mattr3(attr3)
{
    printf("==================== 编译阶段，name = %s, attr1 = %s, attr3 = %f\n", name.c_str(), attr1.c_str(), attr3);
}
    ...
    : mLayerName(name)
{
    ...
    printf("==================== 推理阶段，name = %s, attr1 = %s, attr3 = %f\n", name.c_str(), mattr1.c_str(), mattr3);
}
...
{
    printf(">>>>>>>>>>>>>> getOutputDimensions，name = %s, attr1 = %s, inputs = %d\n", mLayerName.c_str(), mattr1.c_str(), inputs->nbDims);
...
{
    printf(">>>>>>>>>>>>>> enqueue，name = %s, attr1 = %s\n", mLayerName.c_str(), mattr1.c_str());
...
{
    printf(">>>>>>>>>>>>>> serialize，name = %s, attr1 = %s\n", mLayerName.c_str(), mattr1.c_str());
...
{   
    printf(">>>>>>>>>>>>>> supportsFormatCombination，name = %s, attr1 = %s, pos = %d\n", mLayerName.c_str(), mattr1.c_str(), pos);
...
void MySELUPlugin::configurePlugin(const DynamicPluginTensorDesc* in, int32_t nbInputs,
    ...
    printf(">>>>>>>>>>>>>> configurePlugin，name = %s, attr1 = %s\n", mLayerName.c_str(), mattr1.c_str());
    // Validate input arguments
