...
def calcWnrFilter(input_h_PSF, output_G, nsr):
    h_PSF_shifted = np.zeros_like(input_h_PSF)
...
if __name__ == '__main__':
    ...
    img_path = dir_root + "text_defocus.jpg"
    ...
    help()
    imgIn = cv2.imread(args.image, cv2.IMREAD_GRAYSCALE)    # (472, 697)
    if imgIn is None:
        print("ERROR : Image cannot be loaded..!!")
        exit(-1)

    #imgOut = np.empty_like(imgIn)
    roi = (0, 0, imgIn.shape[0] & -2, imgIn.shape[1] & -2)      # (0, 0, 472, 696)

    imgOut = np.zeros(roi[2:], dtype=np.float32)
    #Hw calculation (start)
    Hw = np.zeros(roi[2:], dtype=np.float32)
    h = np.zeros(roi[2:], dtype=np.float32)      # (472, 697)
    calcPSF(h, roi[2:], args.R)
