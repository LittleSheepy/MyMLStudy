import sys
sys.path.insert(0,r'D:\00myGitHub\aFolder_YOLO\02yolov7s\yolov7/')
...
class WindowAttention_v2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = F.linear(input=x, weight=self.qkv.weight, bias=qkv_bias)  # torch.Size([361, 49, 96])
        qkv = qkv.reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        # cosine attention
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01))).exp()   # 211
...
def window_reverse_v2(windows, window_size, H, W):
    ws = int(windows.shape[0])
    B = int(ws / (H * W / window_size / window_size))
    x = windows.view(B, H // window_size, W // window_size, window_size, window_size, -1)
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1)
    return x
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        L = H * W
        x = x.permute(0, 2, 3, 1).contiguous().view(-1, L, C)  # b, L, c
