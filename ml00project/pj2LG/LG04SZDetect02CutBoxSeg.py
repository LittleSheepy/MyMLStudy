    ...
    with open(txt_path, 'r') as file:
        ...
        for line in file:
            ...
            if line == "":
                continue
            data = line.split(" ")
            contourInfo["cls"] = data[0]
            contours = []
            point_cnt = int((len(data)-1)/2)
            for i in range(point_cnt):
                x = int(float(data[i*2+1])*w)
                y = int(float(data[i*2+2])*h)
                contours.append([x, y])
            contourInfo["contours"] = contours
            contoursAll.append(contourInfo)
...
def cutBox(i_dir=0):
    ...
    for imgfile in tqdm(os.listdir(img_src)):
        ...
        for box in algo.vecBox:
            ...
            mask_box = mask[box_y:box_y+iH, box_x:box_x+iW]
            box1 = Box(box_x, box_y, box_x+iW, box_y+iH)
            contours, _ = cv2.findContours(mask_box, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            bEnterFlg = False
            ...
            for contour in contours:
                ...
                for cnt in range(len(contour)):
                    new_x = contour[cnt][0][0]
                    x = round(float(new_x/iW), 6)
                    new_y = contour[cnt][0][1]
                    y = round(float(new_y/iH), 6)
                    line = line + " " + str(x)
                    line = line + " " + str(y)
