...
import os
#os.chdir("D:\myGitHub/tensorflow-yolov3")
import cv2
import random
import numpy as np
...
class Dataset(object):
    ...
    def __init__(self, dataset_type):
        ...
        self.train_input_sizes = config.input_sizes
    ...
    def __next__(self):
        ...
        with tf.device('/cpu:0'):
            ...
            batch_image = np.zeros((self.batch_size, self.train_input_size, self.train_input_size, 3))              # (1, 416, 416, 3)

            batch_label_sbbox = np.zeros((self.batch_size, self.train_output_sizes[0], self.train_output_sizes[0],
                                          self.anchor_per_scale, 5 + self.num_classes))                             # (1, 52, 52, 3, 85)
            batch_label_mbbox = np.zeros((self.batch_size, self.train_output_sizes[1], self.train_output_sizes[1],
                                          self.anchor_per_scale, 5 + self.num_classes))                             # (1, 26, 26, 3, 85)
            batch_label_lbbox = np.zeros((self.batch_size, self.train_output_sizes[2], self.train_output_sizes[2],
                                          self.anchor_per_scale, 5 + self.num_classes))                             # (1, 13, 13, 3, 85)

            batch_sbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4))     # (1, 150, 4)
            batch_mbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4))     # (1, 150, 4)
            batch_lbboxes = np.zeros((self.batch_size, self.max_bbox_per_scale, 4))     # (1, 150, 4)
