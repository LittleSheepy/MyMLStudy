...
from algseg.dataset.chase_db1 import Chase_db1
...
class CChase_db1():
    ...
    def init_dataset(self, dataset_types):
        if "train" in dataset_types:
            self.train_dataset = Chase_db1(root=self.data_root,
                               split='training', transform=self.train_transform)

            self.train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True)

        if "val" in dataset_types:
            self.val_dataset = Chase_db1(root=self.data_root,
        ...
        if "test" in dataset_types:
            self.test_dataset = Chase_db1(root=self.data_root,
