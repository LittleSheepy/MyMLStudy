...
def train():
    ...
    model.train(cfg="ultralytics/yolo/cfg/LG.yaml", epochs=300)

def train_cls():
    model = YOLO(r"D:\08weight\08yolov8\8.2.0\yolov8s-cls.pt")  # build a new model from scratch
    # model = YOLO(r"D:\00myGitHub\aFolder_YOLO\03yolov8s\ultralytics\runs\detect\train11\weights/best.pt")  # build a new model from scratch
    model.train(cfg="ultralytics/cfg/default.yaml", epochs=300)
...
def predict():
    ...
    img_path = r"D:\0\0LG_DATA\SZ_OKNG_0607\img_save_have0608_8\/"         # 0607 1_1
    model.predict(img_path, save=True, save_txt=True, imgsz=320, conf=0.5,
                  line_width=1,
                  save_conf=False
                  )
